...
const op2obj = txt => {
  const t1 = txt.split(/\n/);
  const t2 = t1.reduce((res, row) => {
    row = row.trim();
    if (row === '') return res;
    if (row.slice(0, 1) === '#') return res;
    const els = row.split(/\s+/);
    const name = els.shift();
    const args = [];
    const opcodes1 = [];
    els.map(e => ((e in arglut) ? args : opcodes1).push(e));
    const opcodes = opcodes1.reduce((res, e) => {
      const ee = e.split('=');
      res[ee[0]] = Number(ee[1]);
      return res;
    }, {});
    const instr = {name, args, opcodes};
    instr.type = classify(instr);
    res[name] = instr;
    return res;
  }, {});
  return t2;
