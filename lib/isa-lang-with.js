  ...
  return {
    ...
    token: function (stream, stt) {
      ...
      case 0:
        ...
        break;
      case 1:
        { const m = stream.match(/^(32|64|128)/); if (m) { stt.xlen = Number(m[0]); stt.state = 2; return 'className'; } }
      ...
      case 2: // extension signature
        ...
        { const m = stream.match(/^z([a-z0-9]+[a-oq-z]|[a-z]+)(_|$|\d)/i, false); if (m) {
          const w = stream.match(/^z([a-z0-9]+[a-oq-z]|[a-z]+)/i);
          stt.append({kind: 'z', name: w[0], start}); stt.state = 3;  return 'macroName';
        } }
        { const m = stream.match(/^s(v\d\d|[a-z0-9]+[a-oq-z]|[a-z]+)(_|$|\d)/i, false); if (m) {
          const w = stream.match(/^s(v\d\d|[a-z0-9]+[a-oq-z]|[a-z]+)/i);
          stt.append({kind: 's', name: w[0], start}); stt.state = 3;  return 'propertyName';
        } }
